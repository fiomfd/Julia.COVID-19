...
# Shaanxi
PShaanxi=8.275000;
AShaanxi=parse.(Float64,Array(Wcsv[84,5:qw]))/PShaanxi;
NShaanxi=zeros(DD);
for j=1:DD
...
# Israel
PISR=8.789774;
AISR=parse.(Float64,Array(Wcsv[155,5:qw]))/PISR;
NISR=zeros(DD);
for j=1:DD
    NISR[j]=(AISR[679+j]-AISR[672+j])/7
end
BISR=parse.(Float64,Array(Xcsv[155,5:qw]))/PISR;
NDISR=zeros(DD);
for j=1:DD
    NDISR[j]=(BISR[j+679]-BISR[j+672])/7
end
