...
# Set the SIR-SD system of ordinary differential equations 1
omega=0.5;
kappa=1/2;
Csd=1;
C1=50;
f(u,p,t) = [beta*(N-u[1]-10*u[4])*u[2]; beta*(N-u[1]-10*u[4])*u[2]-gamma*u[2]; gamma*u[2]; omega*(1-10*u[4]/(N-u[1]))*u[4]*tanh(kappa*(-Csd+C1*(1-exp(-beta*u[2]))))];
# Set the initial data
SD0=0.01*(N-T0);
u0=[T0;T0-R0;R0;SD0];
# Set the initial value problem
...
p3=plot(sol, 
     ...
     yaxis="cases",
     legendfont=font(14), 
     label=["total cases" "active cases" "discharged" "SD/10"], 
     palette = :seaborn_bright, 
     legend = :right)
